module.exports = {
    ...
    let curl = child.exec(`curl --header "token: ${config.lewd}" -X POST -F "file=@./image/${fname}" https://lewd.se/upload`)
    ...
    curl.stdout.on("data", function(data) {
        ...
        if(curlout.status !== 200) return msg.channel.send("Error, Please try again later.")

        let sendit = new Discord.RichEmbed()
        .setColor("#"+(Math.random()*0xFFFFFF<<0).toString(16))
        .addField("Here is a link to your image:", `${curlout.data.link}`)
        .setFooter("Thanks to lewd.se")

        fs.unlinkSync(`./image/${fname}`)

        return msg.channel.send(curlout.data.link, sendit)
    })
